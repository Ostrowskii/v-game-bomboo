...
2. Abra o front (GitHub Pages, Vercel etc.). Em `localhost` o cliente usa `ws://localhost:2020`; em qualquer domínio público ele assume `ws://18.228.172.111:8080`.
3. Também é possível sobrescrever o destino adicionando `?ws=wss://meu-endpoint` à URL ou definindo `window.__VIBI_WS_URL__ = "ws://..."` antes de carregar o bundle.
...
- Em qualquer outro host ele aponta para `ws://18.228.172.111:8080` (servidor que você acessa por SSH).
- Adicione `?ws=ws://IP:PORTA` ou defina `window.__VIBI_WS_URL__` para forçar outro backend sem rebuild.
